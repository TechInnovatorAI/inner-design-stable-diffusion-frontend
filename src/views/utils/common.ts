 ...
 * @returns 距离
 */
export const getPositionToLineDistance = (pos: MousePosition, startPos: MousePosition, endPos: MousePosition) => {
  /**
   * 1. 计算三点之间的直线距离
   * 2. 计算三角形半周长
   * 3. 通过海伦公式求面积
...
export const isInsideRect = (
  ...
  return (
    rect.x + rect.width > position.x && position.x > rect.x && rect.y + rect.height > position.y && position.y > rect.y
  )
}
