import { MousePosition } from 'src/context/PaintTypes'
 ...
 */
export const eraserRender = (context: CanvasRenderingContext2D, cleanCanvas: () => void, instance: Eraser) => {
  for (let i = 0; i < instance.positions.length - 1; i++) {
    _cleanLine(instance.positions[i], instance.positions[i + 1], context, cleanCanvas, instance.cleanWidth)
  }
}
...
const _cleanLine = (
  ...
  //获取两个点之间的剪辑区域四个端点
  const asin = (cleanWidth / 1.5) * Math.sin(Math.atan((y2 - y1) / (x2 - x1)))
  const acos = (cleanWidth / 1.5) * Math.cos(Math.atan((y2 - y1) / (x2 - x1)))
  const x3 = x1 + asin
  const y3 = y1 - acos
  const x4 = x1 - asin
  const y4 = y1 + acos
