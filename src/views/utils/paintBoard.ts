import { ElementRect, ELEMENT_INSTANCE, MousePosition } from 'src/context/PaintTypes'
...
export class PaintBoard {
  ...
  constructor(canvas: HTMLCanvasElement, width: number, heigth: number) {
    ...
    this.initCanvasSize(width, heigth)
  ...
  loadMaterial() {
    return new Promise<void>(resolve => {
      ...
      const imgurl = '/images/pattern/crayon.png'
  ...
  recordCurrent(type: string) {
    ...
    switch (type) {
      case CANVAS_ELE_TYPE.FREE_DRAW:
        ele = new FreeDraw(this.currentLineColor, this.currentLineWidth, this.layer.current, this.currentFreeDrawStyle)
        break
  ...
  cache() {
    ...
    const { currentLineColor, currentLineWidth, cleanWidth, originTranslate, layer, version } = this
    const state = {
      currentLineColor,
      currentLineWidth,
      cleanWidth,
  ...
  // 当前画笔宽度
  currentLineWidth = CommonWidth.W8
